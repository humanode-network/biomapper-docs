...
The moment of generation change for the purpuses of the on chain integration is
considrented to be the moment when the `initGeneration` transaction from the
step 3 is included on chain.
Operatons 3-4 are typically bundled into a single transaction.
